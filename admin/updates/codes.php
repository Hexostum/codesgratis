...
WHERE code NOT 
REGEXP '^CGRATIS[A-Z0-9]+'
AND OK=0 AND utilisateur is not null
...
LIMIT 1");
	...
	$true = true;
	if($membre_id>=0):
		$res = mysql_query('SELECT * FROM codesgratis_cgcodes WHERE code NOT REGEXP \'^CGRATIS[A-Z0-9]+\' AND OK=0 AND utilisateur=\''.$utilisateur.'\' LIMIT 20 ');
		//echo mysql_error();
		while($sql_code = mysql_fetch_array($res,MYSQL_ASSOC)):
			...
			$code = "'".$sql_code['code']."'";
				...
				$sql = 'INSERT INTO codesgratis_code VALUES( NULL , '.$code.' , 	'.FP_CGCODES.'  , 	'.$membre_id.'  , 	'.$date.'  ,	'.$date.', NULL ) ';
				if(mysql_query($sql)):
