...
<body>
	...
	<div id="global">
		...
		<div id="corps" style="text-align:center">
			...
			<h1>Tentez votre chance sur l'instant gagnant cliquopass !</h1>
	...
	if($_POST['jouer'] == 1)
	{
		...
		if($donnees['nb_tickets_plus'] >= 1)
		{
			...
			mysql_query('UPDATE codesgratis_cgcodes_plus SET utilisation=\'Instant gagnant cliquopass\' WHERE code=\''. $donnees2['code'] .'\'');
			...
			//Instant gagnant
			mysql_query('UPDATE codesgratis_instants_gagnants_cliquopass SET nb_courant=nb_courant+1');
			$retour3 = mysql_query('SELECT nb_courant FROM codesgratis_instants_gagnants_cliquopass');
			$donnees3 = mysql_fetch_array($retour3);
			if($donnees3['nb_courant'] == 15)
			{
				mysql_query('UPDATE codesgratis_instants_gagnants_cliquopass SET nb_courant=0');
				mysql_query('INSERT INTO codesgratis_instants_gagnants_cliquopass_gagnants VALUES(\'\', \''. $_SESSION['pseudo'] .'\', \''. time() .'\')');
						
				$codes_gagnant = '';
				
				for($i = 1 ; $i <= 5 ; $i++)
				{
					$retour7 = mysql_query('SELECT code FROM codesgratis_instants_gagnants_cliquopass_codes');
					$donnees7 = mysql_fetch_array($retour7);
					$codes_gagnant .= $donnees7['code'] .'\n';
					mysql_query('DELETE FROM codesgratis_instants_gagnants_cliquopass_codes WHERE code=\''. $donnees7['code'] .'\'');
				}
								
				mysql_query("INSERT INTO codesgratis_messagerie VALUES('', 'FLo', '". $_SESSION['pseudo'] ."', 'Vos 5 cliquopass gagnés à l\'instant gagnant !', 'Félicitations à vous ! =)\n\nVos 5 cliquopass :\n\n$codes_gagnant\n\nCordialement,\nFLo.', '0', '". time() ."')");
							
				echo '<p style="color:green">Vous remportez cet instant gagnant ! 5 cliquopass vous ont été 
	...
	echo '<p>Cet instant gagnant permet de gagner 5 cliquopass !</p>';
					
	echo '<p>C\'est le membre qui valide le quinzième ticket+ qui les remporte. Les autres tickets+ joués 
	...
	$dernier_gagnant = mysql_query('SELECT * FROM codesgratis_instants_gagnants_cliquopass_gagnants ORDER BY id DESC LIMIT 1');
	$dernier_gagnant = mysql_fetch_array($dernier_gagnant);
					
	if(!empty($dernier_gagnant['gagnant']))
	{
		echo '<p>Le dernier instant gagnant cliquopass a été remporté 
	...
	if($donnees['nb_tickets_plus'] >= 1)
	{
		$retour7 = mysql_query('SELECT COUNT(*) AS nb_dispos FROM codesgratis_instants_gagnants_cliquopass_codes');
		$donnees7 = mysql_fetch_array($retour7);
		...
		if($donnees7['nb_dispos'] >= 5)
		{
			?>
							...
							<form method="post" action="instant_gagnant_cliquopass.php">
								<input type="hidden" name="jouer" value="1">
								<br>
								<input type="submit" value="Cliquez ici pour enregistrer un ticket+ et tenter de gagner l'instant gagnant cliquopass ! (1 ticket+ requis)">
							</form>
		...
		{
			echo '<p>Il n\'y a pas assez de cliquopass consacrés à l\'instant gagnant pour le 
	...
	else
	{
	  ...
	  <a href="http://www.xiti.com/xiti.asp?s=393972" title="WebAnalytics" target="_top">
				...
				<!--
				Xt_param = 's=393972&p=instant_gagnant_cliquopass';
